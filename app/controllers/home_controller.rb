class HomeController < ApplicationController
  def index
    @cars_display = Car.joins("INNER JOIN car_types ON car_types.id = cars.car_type_id
      ...
      INNER JOIN vehicle_types ON vehicle_types.id = cars.vehicle_type_id").select(
      " cars.id,car_types.id, car_models.id, car_makes.id, car_colours.id, car_bodies.id, car_drives.id, car_documents.id,
        car_cylinders.id, fuel_types.id, car_transmissions.id, vehicle_types.id, cars.car_engine_id, cars.car_name as car_name, 
        ...
        vehicle_types.vehicle as vehicle_type
      "
      ).where(car_type_id: 1)
